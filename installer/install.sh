...
#   - If named arguments are provided, parse them for VAULT_ROLE_ID, VAULT_SECRET_ID, VAULT_TOKEN, and VAULT_TOKEN_LEASE_DURATION.

# Environment Configuration:
#   The environment configuration is retrieved from ../environments/values-${ENVIRONMENT}.yaml.

# Usage Examples:
#   ./install.sh myenv VAULT_ROLE_ID=your-vault-id VAULT_SECRET_ID=your-secret
...
#   ./install.sh myenv VAULT_TOKEN=your-vault-token VAULT_TOKEN_LEASE_DURATION=31536000
...
if [ -n "$VAULT_ROLE_ID" ] && [ -n "$VAULT_SECRET_ID" ]; then
    echo "Authenticating using approle"
    ...
    echo "Authenticating VAULT token"
    create_kubernetes_secret "vault-secrets-operator" \
